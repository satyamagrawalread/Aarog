...
class DescUpdate extends StatefulWidget {
  const DescUpdate({Key key}) : super(key: key);
...
class _descState extends State<desc> {
      ...
      body: ListView(
        ...
        children: <Widget>[
          Form(
            key: _formKey,
            child: Column(
              children: <Widget>[buildTextFormField(), buildTextFormField2()],
            ),
          ),
          SizedBox(
            height: 10,
          ...
          StreamBuilder<QuerySnapshot>(
            stream: db.collection('readmore').snapshots(),
  ...
  void createData() async {
    if (_formKey.currentState.validate()) {
      ...
      await db.collection('readmore').add({'title': ques, 'desc': ans});
      setState(() => id = ref.id);
      print(ref.id);
    }
  }
  ...
  void readData() async {
    DocumentSnapshot snapshot = await db.collection('readmore').doc(id).get();
    // print(snapshot.data['ques']);
  }

  void updateData(DocumentSnapshot doc) async {
    await db.collection('readmore').doc(doc.id);
  }

  void deleteData(DocumentSnapshot doc) async {
    await db.collection('readmore').doc(doc.id).delete();
    setState(() => id = null);
  }
  ...
  Future alertbox(doc) async {
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: <Widget>[
          FlatButton(
            onPressed: () => {deleteData(doc), Navigator.of(ctx).pop()},
